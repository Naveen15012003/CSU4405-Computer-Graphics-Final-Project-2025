...
void processDebugKeys(GLFWwindow* window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_F2) == GLFW_PRESS && !f2Pressed)
    {
        ...
        std::cout << "\n????????????????????????????????????????????????????????????" << std::endl;
        if (enablePCF) {
            std::cout << "?  ???  PCF: ON  - 13x13 EXTREME BLUR (169 samples, 5x radius)  ?" << std::endl;
            std::cout << "?  Shadow edges = VERY WIDE AND EXTREMELY BLURRY               ?" << std::endl;
            std::cout << "?  This should look like a SOFT GRADIENT                       ?" << std::endl;
        } else {
            std::cout << "?  ? PCF: OFF - RAZOR SHARP HARD SHADOWS (1 sample)           ?" << std::endl;
            std::cout << "?  Shadow edges = KNIFE-EDGE SHARP, NO BLUR AT ALL            ?" << std::endl;
            std::cout << "?  This should look like a HARD CUT                            ?" << std::endl;
        }
        std::cout << "????????????????????????????????????????????????????????????\n" << std::endl;
        f2Pressed = true;
    }
